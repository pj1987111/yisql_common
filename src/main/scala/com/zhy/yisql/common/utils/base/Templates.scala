...
object Templates {
   ...
   *   Templates.evaluate(" hello {0} {1} {0}",Seq("jack",""))
   */
  def evaluate(_str: String, parameters: Seq[String]) = {
        ...
        case Some(func) =>
          funcParser.evaluate(func, (executeFunc) => {
            executeFunc.name match {
              case "uuid" => Symbol(UUID.randomUUID().toString.replaceAll("-", ""))
              case "next" =>
                val index = parameters.indexOf(executeFunc.params.head.name)
                if (index != -1) {
                  Symbol(parameters(index + 1))
    ...
    if (parameters.nonEmpty && parameters.head == "_") {
