...
object HttpClientCrawler {
  ...
  def postJson(url: String, params: Map[String, String], useProxy: Boolean = false): String = {
    var response: CloseableHttpResponse = null
    val hc = if (useProxy) httpclientWithpProxy else httpclient
    try {
      val stringEntity = new StringEntity(JSONTool.toJsonStr(params), Charset.forName("utf-8"))
      val httpPost = new HttpPost(url)
